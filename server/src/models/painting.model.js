...
class Painting {
  ...
  async save() {
    try {
      await dbRun(
        `INSERT INTO paintings (painting_id, name, price, likes, description, image, posted, user_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          this.paintingId,
          this.name,
  ...
  static async post(paintingId) {
    try {
      await dbRun(`UPDATE paintings SET posted = TRUE WHERE painting_id = ?`, [
        paintingId,
      ]);
  ...
  static async getImageData(paintingId) {
    const imageData = dbGet(
      `SELECT image FROM paintings WHERE painting_id = ?`,
      paintingId,
    );
    return imageData;
  }
  ...
  static async like(paintingId, userId) {
    try {
      ...
      return {
        success: true,
        message: `Successfully liked painting with id ${paintingId}`,
      };
    } catch (error) {
  ...
  static async findById(paintingId, posted = 1) {
    try {
      const painting = dbGet(
        `SELECT * FROM paintings WHERE painting_id = ? AND posted = ?`,
        paintingId,
        posted,
      );
      return painting;
